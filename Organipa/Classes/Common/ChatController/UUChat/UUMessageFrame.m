...
- (void)setWFMessage:(WFMessage *)message{
    ...
    if (_showTime){
        CGFloat timeY = ChatMargin;
        CGSize timeSize = [[_message.create_time string]  sizeWithFont:ChatTimeFont constrainedToSize:CGSizeMake(300, 100) lineBreakMode:NSLineBreakByWordWrapping];
    ...
    switch (_message.messageType) {
        case UUMessageTypeText:
            contentSize = [_message.content sizeWithFont:ChatContentFont  constrainedToSize:CGSizeMake(ChatContentW, CGFLOAT_MAX) lineBreakMode:NSLineBreakByWordWrapping];
